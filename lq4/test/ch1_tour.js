...
// ★倒す 前は そもそも 調べられない（ボスの ますだから）
T('倒す前は ボスの ます', C.tileAt('old_pipe',10,5)==='B', C.tileAt('old_pipe',10,5));
...
T('討伐後に 調べる 先が 示される', said('オボロが いた あたりを 調べよう'));
// ★案内した ばしょ＝倒した ます で ある こと
T('倒した ますが しらべられる ように なる', C.tileAt('old_pipe',10,5)==='n', C.tileAt('old_pipe',10,5));
...
talk('old_pipe', 10, 6, 'back');
T('かけらを 拾う', C.G.flags.ch1_foundShard===true);
T('炉から 来た 珠だと わかる', said('炉から 来た 珠'), log.join(' / ').slice(0,80));
// ★次に 何を すれば よいか、その場で わかる
T('つぎの 行き先が 出る', said('中層区の 詰所へ 戻って 報告しよう'), log.join(' / ').slice(-70));
