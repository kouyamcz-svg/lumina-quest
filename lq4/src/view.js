...
function initTex(){
  ...
  }
  {
    ...
    // 岩の 側面：地層・ひび。上が 明るく 下が 暗い。
    TEX.dRockSide = patTex((x,px)=>{
      const g=x.createLinearGradient(0,0,0,px);
      g.addColorStop(0,'#e8e8e8'); g.addColorStop(0.5,'#a8a8a8'); g.addColorStop(1,'#565656');
      x.fillStyle=g; x.fillRect(0,0,px,px);
      for(let i=0;i<7;i++){ const yy=R()*px, hh=4+R()*10;          // 地層
        x.fillStyle = R()<0.5 ? 'rgba(255,255,255,0.10)' : 'rgba(0,0,0,0.14)'; x.fillRect(0,yy,px,hh); }
      for(let i=0;i<70;i++){ x.fillStyle='rgba(0,0,0,'+(0.08+R()*0.14)+')';   // ざらつき
        x.fillRect(R()*px|0,R()*px|0,2+(R()*4|0),2+(R()*3|0)); }
      x.strokeStyle='rgba(30,30,30,0.45)'; x.lineWidth=1.2;          // ひび
      for(let i=0;i<7;i++){ let ax=R()*px, ay=R()*px*0.7; x.beginPath(); x.moveTo(ax,ay);
        for(let k=0;k<4;k++){ ax+=(R()-0.5)*20; ay+=6+R()*12; x.lineTo(ax,ay); } x.stroke(); }
    },128);
    // 岩の 上面：ざらついた 岩。縁を 暗く して 面取りに 見せる。
    TEX.dRockTop = patTex((x,px)=>{
      x.fillStyle='#bdbdbd'; x.fillRect(0,0,px,px);
      for(let i=0;i<220;i++){ const v=R();
        x.fillStyle = v<0.5 ? 'rgba(0,0,0,'+(0.06+R()*0.12)+')' : 'rgba(255,255,255,'+(0.08+R()*0.14)+')';
        x.fillRect(R()*px|0,R()*px|0,2+(R()*5|0),2+(R()*5|0)); }
      const v=x.createRadialGradient(px/2,px/2,px*0.25,px/2,px/2,px*0.72);
      v.addColorStop(0,'rgba(0,0,0,0)'); v.addColorStop(1,'rgba(0,0,0,0.42)');
      x.fillStyle=v; x.fillRect(0,0,px,px);
    },128);
    // 石板の 上面（天空・炉）：縁取りの 彫られた 石板。
    TEX.dSlabTop = patTex((x,px)=>{
      x.fillStyle='#c9c9c9'; x.fillRect(0,0,px,px);
      for(let i=0;i<120;i++){ x.fillStyle='rgba(0,0,0,'+(0.03+R()*0.06)+')';
        x.fillRect(R()*px|0,R()*px|0,2+(R()*4|0),2+(R()*4|0)); }
      const b=px*0.1;
      x.fillStyle='rgba(0,0,0,0.28)'; x.fillRect(0,0,px,b*0.6); x.fillRect(0,0,b*0.6,px);          // 外の 縁（影）
      x.fillStyle='rgba(255,255,255,0.35)'; x.fillRect(0,px-b*0.6,px,b*0.6); x.fillRect(px-b*0.6,0,b*0.6,px);
      x.strokeStyle='rgba(0,0,0,0.30)'; x.lineWidth=2; x.strokeRect(b*1.6,b*1.6,px-b*3.2,px-b*3.2);   // 彫り
      x.strokeStyle='rgba(255,255,255,0.35)'; x.lineWidth=1; x.strokeRect(b*1.6+2,b*1.6+2,px-b*3.2,px-b*3.2);
    },128);
  }
...
function buildMap(name){
  ...
  // ★ダンジョン（kind:dgn）の 3D：DQ4リメイク風に 低く 平らな 壁で 通路を 見せる。
  //   ★LQ3の 洞窟は 屋外の 崖（高さ 2〜4、上に 岩の とがり）で、迷路が 見えなかった。
  const skyD = map.theme==='sky';
  const dgn3d = (cave || skyD) && C.WORLD && C.WORLD.MAP_IDS
             && (C.WORLD.MAP_IDS[name]||{}).kind==='dgn' && name!=='shrine_hill';
  // 床の 上面の 高さ（洞窟は 台地 0.5、天空は 0）。仕掛けは ここを 基準に 置く
  const ft = cave ? 0.5 : 0.0;
  const WALL_H = cave ? 1.4 : 1.15;
  // ★地方ごとの 色（壁の 側面の 色あい／上面／床／水）
  const DPAL = {
    ice:  {side:0xffffff, top:0xe4f4fc, floor:0x5d7f9e, water:0xbfe8ff},
    // lump：上に 載せる 岩くずの 色／deco：地方の 飾り
    well: {side:0xc8a078, top:0xa08a6c, floor:0x6e5a44, water:0x7fc8e0, lump:0x7a6a58, deco:'moss'},   // 地下水路：土と 水
    sea:  {side:0x8fc8c0, top:0x8ab8b0, floor:0xb8a882, water:0x4fa8b8, lump:0x6a9a92, deco:'coral'},  // 海蝕洞：砂と 潮
    peak: {side:0xb8b8c0, top:0xa8a8b0, floor:0x76767c, water:0x8fb0d0, lump:0x8a8a92, deco:'snow'},   // 山道：灰色の 岩
    ...
    if(dgn3d){
      ...
      if(ch==='#'){
        ...
        if(!ice && cave){
          ...
          // 岩くず：平らな 上面を 崩す（全部には のせず、6割ほど）
          { const n = hsh(1)<0.25 ? 0 : 1+(hsh(2)*3|0);
            for(let k=0;k<n;k++){
              const sz=0.12+hsh(80+k)*0.16;
              put('dlump',[x+(hsh(84+k)-0.5)*0.7, WALL_H+sz*0.25, y+(hsh(88+k)-0.5)*0.7, hsh(92+k)*6.28, sz, sz*(0.55+hsh(96+k)*0.4)]);
            } }
          const d=dp.deco, s1 = rows[y+1] && rows[y+1][x];
          if(d==='moss' && hsh(21)<0.45)               // 苔：上面の まだら
            put('dmoss',[x+(hsh(22)-0.5)*0.4, WALL_H+0.012, y+(hsh(23)-0.5)*0.4, hsh(24)*6.28, 0.28+hsh(25)*0.2, 1]);
          if(d==='moss' && s1 && s1!=='#' && hsh(26)<0.5)   // 手前の 壁を つたう 水の すじ
            put('ddrip',[x+(hsh(27)-0.5)*0.6, WALL_H*0.55, y+0.505, 0, 1, 1]);
          if(d==='coral' && hsh(31)<0.3){              // サンゴ：上に 小さな 枝
            const n=2+(hsh(32)*3|0);
            for(let k=0;k<n;k++) put('dcoral',[x+(hsh(33+k)-0.5)*0.5, WALL_H+0.14, y+(hsh(40+k)-0.5)*0.5, hsh(47+k)*6.28, 1, 0.8+hsh(54+k)*0.6]);
          }
          if(d==='coral' && hsh(61)<0.35)              // 貝がら
            put('dshell',[x+(hsh(62)-0.5)*0.5, WALL_H+0.04, y+(hsh(63)-0.5)*0.5, hsh(64)*6.28, 1, 1]);
          if(d==='snow' && hsh(71)<0.35){              // 雪の 吹きだまり：大小の 塊を 寄せて 不規則な 形に
            const bx=x+(hsh(72)-0.5)*0.3, bz=y+(hsh(73)-0.5)*0.3, n=3+(hsh(74)*3|0);
            for(let k=0;k<n;k++){
              const r=0.08+hsh(100+k)*0.14;
              put('dsnow',[bx+(hsh(106+k)-0.5)*0.42, WALL_H+0.01, bz+(hsh(112+k)-0.5)*0.42, 0, r, r*0.35]);
            }
          }
        }
        if(ice){
      ...
      if(ch==='K'){ fl(); put('dgate',[x,ft+0.45,y]); continue; }      // 隔壁
      if(ch==='L'||ch==='l'){                                          // 灯り
        fl(); put('dped',[x,ft+0.25,y]);
        put(ch==='l'?'dorbOn':'dorbOff',[x,ft+0.66,y]); continue;
      }
      if(ch==='O'){ fl(); put('drock',[x,ft+0.38,y]); continue; }      // 押す 岩
      if(ch==='x'){ put('dpit',[x,ft-0.46,y]); continue; }             // 穴
      // ★奥への 口／出口の 門：暗い 入口。両わきに 柱、上に 梁。
      if(ch==='D'||ch==='G'){
        put('dpit',[x,ft-0.10,y]);
        put('dpost',[x-0.42,ft+0.45,y]); put('dpost',[x+0.42,ft+0.45,y]);
        put('dbeam',[x,ft+1.05,y]);
        continue;
      }
      // ★天空の 仕掛け
      if(ch==='p'){ fl(); put('dpipeH',[x,ft+0.45,y]); continue; }     // 光珠管（よこ）
      if(ch==='q'){ fl(); put('dpipeV',[x,ft+0.45,y]); continue; }     // 光珠管（たて）
      if(ch==='h'){ fl(); put('djoint',[x,ft+0.35,y]); continue; }     // 空の 継ぎ目
      if(ch==='H'){ fl(); put('djointOn',[x,ft+0.35,y]); continue; }   // 管を さした 継ぎ目
      if(ch==='u'){ fl(); put('dpiece',[x,ft+0.10,y]); continue; }     // 床の 管の きれはし
      if(ch==='<'||ch==='>'){                                          // 階段
        fl(); put('dstep',[x,ft+0.06,y+0.25]); put('dstep',[x,ft+0.18,y]); put('dstep',[x,ft+0.30,y-0.25]);
        continue;
      }
    }
    if(ch==='w'){                    // みず
  ...
  if(dgn3d){
    ...
    if(!ice && cave){
      addInstanced(new THREE.DodecahedronGeometry(1,0),
        new THREE.MeshPhongMaterial({color: dp.lump, specular:0x222222, shininess:8, flatShading:true}), G_.dlump||[]);
      const mg = new THREE.CircleGeometry(1,7); mg.rotateX(-Math.PI/2);
      addInstanced(mg, new THREE.MeshLambertMaterial({color:0x4f7a3a}), G_.dmoss||[]);
      addInstanced(new THREE.BoxGeometry(0.05,WALL_H*0.8,0.02),
        new THREE.MeshBasicMaterial({color:0x9fd8f0, transparent:true, opacity:0.55}), G_.ddrip||[]);
      addInstanced(new THREE.ConeGeometry(0.035,0.28,5),
        new THREE.MeshPhongMaterial({color:0xf07a8a, specular:0x442222, shininess:20, flatShading:true}), G_.dcoral||[]);
      const sg = new THREE.SphereGeometry(0.07,8,5,0,Math.PI*2,0,Math.PI/2);
      addInstanced(sg, new THREE.MeshPhongMaterial({color:0xf2e6d2, specular:0x888888, shininess:40}), G_.dshell||[]);
      addInstanced(new THREE.SphereGeometry(1,10,6),
        new THREE.MeshLambertMaterial({color:0xe8eef6}), G_.dsnow||[]);
    }
    if(ice){
    ...
    const wt = TEX.water.clone(); wt.wrapS=wt.wrapT=THREE.RepeatWrapping; wt.needsUpdate=true;
    const wmat = new THREE.MeshBasicMaterial({map:wt, transparent:true, opacity:0.82, color: dp.water});
    const wim = addInstanced(wg, wmat, G_.dwater||[]);
    if(wim){ waters.push({material:wmat, isInst:true}); }
    addInstanced(new THREE.BoxGeometry(1,0.2,1), new THREE.MeshLambertMaterial({color:0x0c1a28}), G_.dwbed||[]);
    addInstanced(new THREE.BoxGeometry(0.96,0.9,0.96),
      new THREE.MeshLambertMaterial({color: ice?0xa8e0ff:0x6a5a48, transparent:ice, opacity: ice?0.78:1}), G_.dgate||[]);
    addInstanced(new THREE.CylinderGeometry(0.2,0.27,0.5,8), new THREE.MeshLambertMaterial({color:0x7a7f88}), G_.dped||[]);
    addInstanced(new THREE.SphereGeometry(0.17,12,8), new THREE.MeshLambertMaterial({color:0x2e3a4c}), G_.dorbOff||[]);
    addInstanced(new THREE.SphereGeometry(0.19,12,8), new THREE.MeshBasicMaterial({color:0xffe6a0}), G_.dorbOn||[]);
    addInstanced(new THREE.DodecahedronGeometry(0.42,0), new THREE.MeshLambertMaterial({color:0x6b6f78}), G_.drock||[]);
    addInstanced(new THREE.BoxGeometry(1,0.08,1), new THREE.MeshBasicMaterial({color:0x020204}), G_.dpit||[]);
    addInstanced(new THREE.BoxGeometry(0.16,1.1,0.3), new THREE.MeshLambertMaterial({color: dp.top}), G_.dpost||[]);
    addInstanced(new THREE.BoxGeometry(1.0,0.2,0.34), new THREE.MeshLambertMaterial({color: dp.top}), G_.dbeam||[]);
    // 天空の 仕掛け
    const brass = new THREE.MeshLambertMaterial({color:0xc8a050});
    const pgH = new THREE.CylinderGeometry(0.16,0.16,1,10); pgH.rotateZ(Math.PI/2);
    const pgV = new THREE.CylinderGeometry(0.16,0.16,1,10); pgV.rotateX(Math.PI/2);
    addInstanced(pgH, brass, G_.dpipeH||[]);
    addInstanced(pgV, brass, G_.dpipeV||[]);
    addInstanced(new THREE.BoxGeometry(0.5,0.7,0.5), new THREE.MeshLambertMaterial({color:0x4a4038}), G_.djoint||[]);
    addInstanced(new THREE.BoxGeometry(0.5,0.7,0.5), new THREE.MeshBasicMaterial({color:0xffd870}), G_.djointOn||[]);
    const pgP = new THREE.CylinderGeometry(0.12,0.12,0.6,8); pgP.rotateZ(Math.PI/2);
    addInstanced(pgP, brass, G_.dpiece||[]);
    addInstanced(new THREE.BoxGeometry(0.9,0.12,0.26), new THREE.MeshLambertMaterial({color: dp.top}), G_.dstep||[]);
    addInstanced(new THREE.BoxGeometry(1,0.3,1), new THREE.MeshBasicMaterial({color:0xf4f6ff, transparent:true, opacity:0.85}), G_.dcloud||[]);
  }
  if(cave){
...
function watchTiles(){
  ...
  if(tileSigMap===curMap && tileSig!==null && sig!==tileSig){
    tileSig = sig;
    buildMap(curMap);
    setActors(true);
    return;
  }
  tileSig = sig; tileSigMap = curMap;
}
