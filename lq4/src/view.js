...
function buildMap(name){
  ...
  const DPAL = {
    ...
    well: {side:0xd8b48c, top:0x7a6650, floor:0x6e5a44, water:0x7fc8e0},   // 地下水路：土と 水
    sea:  {side:0x9fd8d0, top:0x5a8a84, floor:0xb8a882, water:0x4fa8b8},   // 海蝕洞：砂と 潮
    peak: {side:0xc0c0c8, top:0x8a8a90, floor:0x76767c, water:0x8fb0d0},   // 山道：灰色の 岩
    rock: {side:0xc8b8a0, top:0x6a5e50, floor:0x8a7a6a, water:0x7fb8d8},
    sky:  {side:0xf2eee2, top:0xd8cfae, floor:0xbab39c, water:0xffffff},   // 天空：白い 石と 金
    forge:{side:0xd8b8a0, top:0x8a5a40, floor:0x7a6050, water:0xffffff},   // 炉：赤茶の 煉瓦
  };
  const dkey = ice ? 'ice' : /^well/.test(name) ? 'well' : /^sea/.test(name) ? 'sea'
             : /^peak|^zenos/.test(name) ? 'peak' : /^furnace/.test(name) ? 'forge'
             : skyD ? 'sky' : 'rock';
  const dp = DPAL[dkey];
