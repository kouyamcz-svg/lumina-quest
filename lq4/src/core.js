...
const ENEMIES = [
  ...
  // 北の 島（雪原）
  {key:'shimogami',   name:'霜がみ狼',  hp:160,atk:41, def:24, agi:30, exp:540, gold:560, minLv:18,
   art:'shimogami', inflict:{type:'freeze', p:0.16}},
  {key:'itega',       name:'凍て蛾',    hp:150,atk:38, def:22, agi:28, exp:520, gold:540, minLv:18,
   art:'itega', inflict:{type:'sleep', p:0.16}},
  // 東の 島（砂漠）
  {key:'sunamogura',  name:'砂もぐら',  hp:235,atk:45, def:38, agi:12, exp:680, gold:720, minLv:19,
   art:'sunamogura'},
  {key:'hikarabi',    name:'干からび蠍',hp:200,atk:48, def:32, agi:22, exp:660, gold:700, minLv:19,
   art:'hikarabi', inflict:{type:'slow', p:0.18}},
  // 南の 島（平原と 砂浜）
  {key:'honeyadokari',name:'ほね宿かり',hp:265,atk:46, def:46, agi:10, exp:720, gold:760, minLv:20,
   art:'honeyadokari'},
  {key:'nakigai',     name:'鳴き貝',    hp:215,atk:45, def:34, agi:14, exp:690, gold:730, minLv:20,
   art:'nakigai', inflict:{type:'confuse', p:0.18}},
  // 西の 島（平原と 山脈・霊峰）
  {key:'iwainoshishi',name:'岩いのしし',hp:285,atk:52, def:40, agi:18, exp:800, gold:840, minLv:21,
   art:'iwainoshishi'},
  {key:'kodamadori',  name:'こだま鳥',  hp:225,atk:50, def:30, agi:30, exp:780, gold:800, minLv:21,
   art:'kodamadori', inflict:{type:'confuse', p:0.18}},
  // 中央の 島（平原と 森・トロス）
  {key:'hagureookami',name:'はぐれ狼',  hp:265,atk:55, def:34, agi:32, exp:860, gold:880, minLv:22,
   art:'hagureookami'},
  {key:'hoshikabuto', name:'星かぶと',  hp:305,atk:54, def:48, agi:14, exp:900, gold:920, minLv:22,
   art:'hoshikabuto'},
...
const MIDBOSS = {
  ...
  // ---- 第3章 中ボス：あくむへん（氷窟）----
  akumuhen4:{key:'akumuhen4', name:'あくむへん', hp:3000, atk:80, def:42, agi:24, acts:1,
    exp:2800, gold:3000, art:'akumuhen', scale:1.05,
    skill:{p:0.28, mul:1.25, name:'岩を 裂く 爪'},
    inflict:{type:'slow', p:0.20}},
...
const byMap = {
  ...
  peak_path: ['iwafukurou','ishikui','honetori','kareedano','sennengoke'],
};
...
const MAPS = {
  ...
  //     そこへ 五地方だけを おいて ある。
  // ★北の 島の 北東（上2行・x62〜71）は 海。北の 島から 舟で 東の 島へ 抜ける 海峡。
  //   ★まえは 山が 地図の 端まで 続き、東の 島の 北の 海が 閉じて いた（東の 島まで 160歩 → 66歩）
  ground:{name:'地上', theme:'world', enc:true, encRate:0.06, encGrace:6,
    // ★島ごとの 出現表。at の ますと 陸続きの 所に 出る。unique が ひとつも 出せない ときは byMap.ground
    encIslands:[
      {name:'北の 島',  at:[48,8],  unique:['shimogami','itega'],
       pool:['shimogami','itega','nemurishika','kirimatoi']},
      {name:'東の 島',  at:[86,41], unique:['sunamogura','hikarabi'],
       pool:['sunamogura','hikarabi','doronote','kareedano']},
      {name:'南の 島',  at:[54,59], unique:['honeyadokari','nakigai'],
       pool:['honeyadokari','nakigai','kirimatoi','honetori']},
      {name:'西の 島',  at:[9,30],  unique:['iwainoshishi','kodamadori'],
       pool:['iwainoshishi','kodamadori','kareedano','honetori']},
      {name:'中央の 島',at:[48,31], unique:['hagureookami','hoshikabuto'],
       pool:['hagureookami','hoshikabuto','nemurishika','doronote','honetori']},
    ],
    tiles:[
    "~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~",
    "~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~______~~~~~~~~~~_____~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~",
    ...
    "~~_.,.X.,^^^^^^^^^......._~~~~_....,__~~~~~_........_^^^^.,....,.^^^^^^^^:::wwwwrrrrrrV:::^::_~~",
    "~~~_.......^^^^^........._~~~~_....,.,_~~~~~_........_______....^.^^^^^^^::::wwwX::::::::::::_~~",
    "~~~_....................._~~~~_...,...,_~~~~_......,......._.......^^^^^^::::ww::::::::::::::_~~",
    "~~~~_...................._~~~~_......._~~~~~_.......,......_..^..^^^^^^^^..::::^::::^::::::::_~~",
    "~~~~~_......,...........__~~~~_........_~~~~_.............._..^^.^^^^^^^^^.:::::::::::::::::_~~~",
    ...
    warpsXY:{
      ...
      '9,29':{to:'peak_village', x:10, y:13},
      '54,58':{to:'coral_bay', x:11, y:13},
      '80,32':{to:'well_west', x:10, y:11},    // ★湧き水 西の集落（LQ3 の ザールの 所）
      '86,40':{to:'well_town', x:12, y:13},    // ★湧き水 東の集落（LQ3 の オアシスの サーラの 所）
      '80,41':{to:'well_cave', x:11, y:27},    // ★ふたつの 集落の あいだ、オアシスの ほとりの 井戸,
      '48,30':{to:'toros', x:11, y:13},
      '53,31':{to:'shrine_hill', x:10, y:10},
      '10,31':{to:'zenos_cave', x:7, y:7},
      '6,40':{to:'peak_path', x:9, y:31},
      '53,9' :{to:'ice_cave', x:10, y:26}
    }},
  ...
  ice_cave:{name:'氷窟', theme:'ice', enc:true, encRate:0.045, encGrace:6, tiles:[
    "#######################",
    "#######################",
    "###########D###########",
    "###########K###########",
    ...
    "##########.############"],
    warpsXY:{
      '10,27':{to:'ground', x:53, y:10, back:true},
      '11,2' :{to:'ice_core', x:9, y:13}
    }},

  // ★氷の 奥：リーゼが いる ところ（入口からは 見えない）
  ice_core:{name:'氷の 奥', theme:'ice', enc:false, tiles:[
    "###################",
    "###################",
    "###.............###",
    "##...............##",
    ...
    "#########.#########"],
    warpsXY:{
      '9,14':{to:'ice_cave', x:11, y:4, back:true}
    }},

  // ============ 第3章：トロスの村（父の 故郷）============
  toros:{name:'トロスの村', theme:'world', enc:false, tiles:[
    "#######################",
    "#.....................#",
    "#..H..H.........H..H..#",
    "#..I..S.........H..P..#",
  ...
  shrine_hill:{name:'祠の丘', theme:'world', enc:false, tiles:[
    ...
    "##########.##########"],
    warpsXY:{
      '10,11':{to:'ground', x:53, y:32, back:true}
    }},

  // ============ 第3章：若き霊峰（後の エルデ）============
  // ★若き霊峰 は 山の 名（後の エルデ）。町の 名は 霊峰の村。LQ3 の エルデと おなじ 西に ある
  peak_village:{name:'霊峰の村', theme:'ice', enc:false, tiles:[
    ...
    "##########G##########"],
    warpsXY:{
      '10,14':{to:'ground', x:9, y:30}
    }},

  // ★山道：ゼノスの 庵へ つづく 岩の 道
  peak_path:{name:'山道', theme:'cave', enc:true, encRate:0.045, encGrace:6, tiles:[
    "#########D###########",
    "#########K###########",
    "#########.###########",
    "#.....#...#.....#...#",
    ...
    "#########.###########"],
    warpsXY:{
      '9,32':{to:'ground', x:6, y:41, back:true},
      '9,0' :{to:'peak_core', x:9, y:13}
    }},

  well_core:{name:'水脈の 奥', theme:'cave', enc:false, tiles:[
    "###################",
    "###################",
    "##~~...........~~##",
    "#~~.............~~#",
    ...
    "#########.#########"],
    warpsXY:{
      '9,14':{to:'well_cave', x:11, y:3, back:true}
    }},

  sea_core:{name:'潮だまりの 奥', theme:'cave', enc:false, tiles:[
    "#####################",
    "#####################",
    "###~~~.........~~~###",
    "##~~.............~~##",
    ...
    "##########.##########"],
    warpsXY:{
      '10,14':{to:'sea_cave', x:13, y:3, back:true}
    }},

  peak_core:{name:'山頂', theme:'ice', enc:false, tiles:[
    "###################",
    "###################",
    "####...........####",
    "###.............###",
    ...
    "#########.#########"],
    warpsXY:{
      '9,14':{to:'peak_path', x:9, y:3, back:true}
    }},
  ...
  zenos_cave:{name:'山肌の洞', theme:'cave', enc:false, tiles:[
    ...
    "#######.#######"],
    warpsXY:{
      '7,8':{to:'ground', x:10, y:30, back:true}
    }},

  // ============ 第3章：湧き水の町（後の ザール）============
  // ★湧き水 西の集落。まえは 東の 集落と 一つの 町に 同居して いた（東の 町なのに 西の 長が いた）
  well_west:{name:'湧き水 西の集落', theme:'desert', enc:false, tiles:[
    "#####################",
    "#...................#",
    "#..H..H.......H..H..#",
    "#..H..H.......H..H..#",
    ...
    "##########G##########",
      ...
      '10,12':{to:'ground', x:80, y:33}
    }},
  // ★湧き水 東の集落（東の 砂漠。LQ3 の オアシスの サーラの 所）
  well_town:{name:'湧き水 東の集落', theme:'desert', enc:false, tiles:[
    "#########################",
    "#.......................#",
    "#..H..H...........H..H..#",
    "#..I..S...........H..P..#",
    ...
    "############G############"],
    warpsXY:{
      '12,14':{to:'ground', x:86, y:41}
    }},

  // ★地下水路：第三の 水脈を さがす
  well_cave:{name:'地下水路', theme:'cave', enc:true, encRate:0.045, encGrace:6, tiles:[
    "###########D###########",
    "###########K###########",
    "###########.###########",
    "#.......~.......~...#.#",
    ...
    "###########.###########"],
    warpsXY:{
      '11,28':{to:'ground', x:80, y:40, back:true},
      '11,0' :{to:'well_core', x:9, y:13}
    }},
  ...
  coral_bay:{name:'珊瑚の入り江', theme:'world', enc:false, tiles:[
    ...
    warpsXY:{
      ...
      '11,7' :{to:'sea_cave', x:13, y:23}
    }},

  // ★海蝕洞：ナミが 夢を 視た ところ
  sea_cave:{name:'海蝕洞', theme:'cave', enc:true, encRate:0.045, encGrace:6, tiles:[
    "#############D#############",
    "#############K#############",
    "#############.#############",
    "#.#...........#...........#",
    ...
    "#############.#############"],
    warpsXY:{
      '13,24':{to:'coral_bay', x:11, y:7, back:true},
      '13,0' :{to:'sea_core', x:10, y:13}
    }},
...
const SHOPS = {
  ...
  'ice_camp:S':[
    {kind:'h',  name:'薬草',            v:20, price:10},
    {kind:'wtr',name:'魔法の 聖水',     v:30, price:70},
    {kind:'w',  name:'氷牙の 剣',       v:19, price:2200},
    {kind:'w',  name:'氷牙の 槍',       v:21, price:2400},
    {kind:'a',  name:'毛皮の 胴着',     v:17, price:1900},
    {kind:'a',  name:'毛皮の 外套',     v:13, price:1500},
  ],
  'well_town:S':[
    {kind:'h',  name:'薬草',            v:20, price:10},
    {kind:'wtr',name:'魔法の 聖水',     v:30, price:70},
    {kind:'w',  name:'隊商の 曲刀',     v:21, price:2800},
    {kind:'w',  name:'水晶の 杖',       v:12, price:2000},
    {kind:'a',  name:'革の 鎧',         v:19, price:2600},
    {kind:'a',  name:'砂除けの 衣',     v:14, price:1800},
  ],
  'coral_bay:S':[
    {kind:'h',  name:'薬草',            v:20, price:10},
    {kind:'wtr',name:'魔法の 聖水',     v:30, price:70},
    {kind:'w',  name:'珊瑚の 槍',       v:24, price:3400},
    {kind:'w',  name:'潮の 杖',         v:14, price:2600},
    {kind:'a',  name:'鱗の 胸当て',     v:21, price:3200},
    {kind:'a',  name:'海女の 衣',       v:16, price:2400},
  ],
  'peak_village:S':[
    {kind:'h',  name:'薬草',            v:20, price:10},
    {kind:'wtr',name:'魔法の 聖水',     v:30, price:70},
    {kind:'w',  name:'岩鉄の 剣',       v:24, price:3800},
    {kind:'w',  name:'山の 錫杖',       v:16, price:3000},
    {kind:'a',  name:'岩鉄の 鎧',       v:23, price:3800},
    {kind:'a',  name:'白絹の 装束',     v:18, price:2800},
  ],
  'toros:S':[
    {kind:'h',  name:'薬草',            v:20, price:10},
    {kind:'wtr',name:'魔法の 聖水',     v:30, price:70},
    {kind:'w',  name:'鍛冶村の 剣',     v:26, price:4400},
    {kind:'w',  name:'鍛冶村の 槍',     v:28, price:4800},
    {kind:'a',  name:'鍛冶村の 鎧',     v:25, price:4400},
    {kind:'a',  name:'鍛冶村の 衣',     v:19, price:3200},
  ],
  'upper_dist:S':[
    {kind:'h',  name:'薬草',            v:20, price:10},
    {kind:'wtr',name:'魔法の 聖水',     v:30, price:70},
...
const INN_PRICE = {lower_dist:6, mid_dist:18, upper_dist:40,
  sky_port:40, ice_camp:50, well_town:55, coral_bay:60, peak_village:65, toros:70};
// ---------------- 状態 ----------------
...
function wardMsg(to){
  ...
  // ★同じ 章の うちでも、順を とばして 着いた ときは
  //   「なぜ 入れないか」を つたえる（earlyInChapter を 書いた ところだけ）。
  //   ★ふねで 好きな 順に 行ける ように なり、先に 着ける ように なった。
  if(wd.earlyInChapter && wd.flag && !G.flags[wd.flag]) return wd.earlyInChapter;
...
function doWarp(w){
  ...
  if(!w.back && !fromInner && !w.escape){
    ...
    G.entry[w.to] = {map:P.map, x:back.x, y:back.y};
  }
  G.busy=true;
  if(A.door) A.door();
  V.fade(1, ()=>{
...
function openChest(x,y){
  ...
  // ★章データの treasure に 中身が 書いて あれば それを 出す。
  //   ★乱数だけ だった ため、ダンジョンの 奥の 箱でも 薬草 4つ などで、
  //     ざこ 1戦の おかねと 変わらなかった。
  const fixed = ((chData()||{}).treasure||{})[key];
  if(fixed){
    const lines = ['宝箱を 開けた！'];
    if(fixed.kind==='w' || fixed.kind==='a'){
      // 装備は 持ち物に 入れる（いまの 装備より 強ければ その場で 着る）
      const who = party.find(m=>m.cls===fixed.for) || party[0];
      const slot = fixed.kind==='w' ? 'weapon' : 'armor';
      const cur = who[slot];
      lines.push(fixed.name+'を てにいれた！');
      if(!cur || (cur.v||0) < fixed.v){
        who[slot] = {kind:fixed.kind, name:fixed.name, v:fixed.v};
        lines.push(who.name+'が '+fixed.name+'を 身に つけた。');
      }else{
        const price = Math.floor((fixed.price||fixed.v*120)/2);
        P.gold += price;
        lines.push('（いまの 装備の ほうが 強い。'+price+'ゴールドに かえた）');
      }
    }else if(fixed.kind==='g'){
      P.gold += fixed.v; lines.push(fixed.v+'ゴールドを てにいれた！');
    }else if(fixed.kind==='h'){
      P.herbs += fixed.v; lines.push('薬草を '+fixed.v+'つ てにいれた！');
    }else if(fixed.kind==='wtr'){
      P.waters += fixed.v; lines.push('魔法の 聖水を '+fixed.v+'つ てにいれた！');
    }
    U.msg(lines, ()=>{ U.hud(); G.mode='field'; });
    return;
  }
  const ch = G.chapter || 1;
  const mul = [1, 1, 6, 18, 30][Math.min(ch, 4)] || 30;
  const r=Math.random();
  if(r<0.45){
...
// ★敵の 絵が あるか（絵の ない 敵は 出さない）
function hasFoeArt(k){
  const e = ENEMIES.find(x=>x.key===k); if(!e) return false;
  const M = (typeof MON!=='undefined') ? MON : null;
  return !!(M && (M[e.art] || M[e.key]));
}
// ★島（陸続きの かたまり）を 求める。地図が かわらない かぎり 覚えておく
const islandCache = {};
function islandIdAt(map, x, y){
  const m = MAPS[map]; if(!m) return -1;
  const sig = m.tiles.length+':'+m.tiles.join('').length;
  let c = islandCache[map];
  if(!c || c.sig!==sig){
    const H=m.tiles.length, W=m.tiles[0].length, id=Array.from({length:H},()=>Array(W).fill(-1)); let n=0;
    for(let yy=0;yy<H;yy++)for(let xx=0;xx<W;xx++){
      if(id[yy][xx]>=0 || !walkable(map,xx,yy)) continue;
      const q=[[xx,yy]]; id[yy][xx]=n;
      for(let i=0;i<q.length;i++){ const [a,b]=q[i];
        for(const [dx,dy] of [[1,0],[-1,0],[0,1],[0,-1]]){ const u=a+dx,v=b+dy;
          if(u>=0&&v>=0&&u<W&&v<H && id[v][u]<0 && walkable(map,u,v)){ id[v][u]=n; q.push([u,v]); } } }
      n++;
    }
    c = islandCache[map] = {sig, id};
  }
  return (c.id[y]||[])[x] ?? -1;
}
function islandAt(map, x, y){
  const m = MAPS[map]; if(!m || !m.encIslands) return null;
  const me = islandIdAt(map, x, y); if(me<0) return null;
  return m.encIslands.find(z=>islandIdAt(map, z.at[0], z.at[1])===me) || null;
}
function islandPool(map, x, y){
  const z = islandAt(map, x, y); if(!z) return null;
  const pool = z.pool.filter(hasFoeArt);
  return z.unique.some(k=>pool.includes(k)) ? pool : null;
}
function startBattle(kind){
  ...
  if(kind){
    ...
    let all = islandPool(P.map, P.x, P.y) || byMapCh[(G.chapter||1)+':'+P.map] || byMap[P.map];
    if(!all){
      const m=MAPS[P.map];
...
const WARDS = {
  ...
  upper_dist: {chapter:3, flag:'ch2_taskOpen',
    msgEarly:['上層への 石段。白い 手すりが 雲まで 続いている。',
              ...
              '衛兵「……お嬢様。今は そういう ご時世で」'],
    msg:['衛兵「炉の 件だな。通れ」']},
  // ★第3章：五地方は ふねで 好きな 順に 行けるが、話は 順に すすむ。
  //   先に 着いても 何も できない ので、行き先を つたえる。
  well_town: {chapter:4, flag:'ch3_iceDone',
    earlyInChapter:['湧き水 東の集落。広場に 人が 集まって いる。',
              '町の 者「空の 者か。……今年は まだ 荷が 来とらんが」',
              'セレン「順に 回って います。まだ 北の 谷の ぶんが」',
              'イオ「……先に 氷の谷を 片づけましょう」'],
    msg:['湧き水 東の集落。西の 集落と にらみあって いる。']},
  well_west: {chapter:4, flag:'ch3_iceDone',
    earlyInChapter:['湧き水 西の集落。砂除けの 布が 張られて いる。',
              '集落の 者「空の 者か。……今年は まだ 荷が 来とらんが」',
              'セレン「順に 回って います。まだ 北の 谷の ぶんが」',
              'イオ「……先に 氷の谷を 片づけましょう」'],
    msg:['湧き水 西の集落。東の 集落と にらみあって いる。']},
  well_cave: {chapter:4, flag:'ch3_iceDone',
    earlyInChapter:['オアシスの ほとりに 古い 井戸が ある。',
              'セレン「集落の 人に 話を 聞いてからに しましょう」'],
    msg:['井戸の 底へ 下りた。']},
  coral_bay: {chapter:4, flag:'ch3_caravan',
    earlyInChapter:['珊瑚の入り江。浜に 舟が 並んで いる。',
              '浜の 者「空の 方。荷は まだですかい」',
              'セレン「東の 町が 先です。順が あるの」',
              'イオ「……戻りましょう」'],
    msg:['珊瑚の入り江。潮の 匂いが する。']},
