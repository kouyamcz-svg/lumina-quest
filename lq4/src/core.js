...
const ENEMIES = [
  ...
  {key:'honokage',   name:'ほのかげ',     hp:88, atk:22, def:18, agi:12, exp:1900, gold:320, minLv:12,
   art:'kagekakera', inflict:{type:'confuse', p:0.12}},
  {key:'sumibane',   name:'すみばね',     hp:80, atk:20, def:16, agi:20, exp:1750, gold:300, minLv:12,
   art:'akumuga'},
  {key:'roheifu',    name:'炉の へいふ',  hp:104,atk:24, def:22, agi:10, exp:2200, gold:380,minLv:14,
   art:'shokudai'},
...
const MIDBOSS = {
  ...
  fornax:{key:'fornax', name:'あくむじゅう フォルナクス', hp:1280, atk:46, def:26, agi:16, acts:1,
  ...
  // ---- 第2章 中ボス：ひばしら（炉の 管に 巣くう）----
  hibashira:{key:'hibashira', name:'ひばしら', hp:760, atk:42, def:24, agi:18, acts:2,
    exp:520, gold:300, art:'kanmure', scale:1.05,
    skill:{p:0.30, mul:1.20, name:'ねつの むち'},
...
const byMap = {
  ...
  furnace:   ['honokage','sumibane','roheifu','hakoyami'],
};
...
const MAPS = {
  ...
  upper_dist:{name:'上層区', theme:'sky', enc:false, tiles:[
    ...
    warpsXY:{
      ...
      '16,9' :{to:'furnace',   x:10, y:19},
      '1,7'  :{to:'garden',    x:9,  y:13}
    }},
...
const SOLID = new Set(['#','f','w','o','T','F','K','~','^','H','e','y','j',
                       'O','x','L','l','p','q','%']);   // p/q＝光珠管、%＝花壇（かべ あつかい）
// ★しかけ用の タイル（IVから）
...
const WARDS = {
  ...
  furnace: {chapter:3, flag:'ch2_noeJoined',
    msgEarly:['昇降機の 前に 錠が おりている。',
              'イオ「……上層の 者しか 動かせないのか」'],
    msg:['昇降機の 前に 錠が おりている。',
         '主任「案内を つけると 言ったろう。庭園に いる」',
         'イオ「……先に 庭園ですね」']},
