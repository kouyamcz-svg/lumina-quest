...
function wardMsg(to){
  const wd = WARDS[to];
  if(!wd) return null;
  // ★まだ 入れない とき（章が 前／めじるしが 立って いない）は
  //   「なぜ 入れないか」を つたえる。
  //   ★ふねで 好きな 順に 行ける ように なり、先に 着く ことが できる。
  if(wd.msgEarly && ((G.chapter||1) < wd.chapter || (wd.flag && !G.flags[wd.flag])))
    return wd.msgEarly;
