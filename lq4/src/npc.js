...
const NPCS = {
  ...
  upper_dist: [
    {at:'9,4', spr:'villagerB', name:'炉の 主任', lines:[
      ...
      {when:{flag:'ch2_towerPaid'}, text:[
        '主任「雲の 記録は 受け取った。昇降機は 東の 館だ」',
        '主任「これで『炉に 異常あり』で 通る」']},
      {when:{flag:'ch2_noeJoined'}, text:[
        '主任「案内は 見つかったな。だが 手続きが 先だ」',
        '主任「雲見の 塔の 記録を 持ってこい。庭園の 西はしだ」']},
  ...
  garden: [
    ...
    {at:'13,5', spr:'noe', name:'夢守りの ノエ', lines:[
      ...
      // ★主任に 会う 前だと ここで 止まる。何を すれば よいか 言う。
      {when:{flag:'ch2_taskTaken'}, text:['外套の 少年が 花の 影を 見ている。']},
  ...
  tower1: [
    ...
    {at:'12,6', spr:'villagerA', name:'雲の 記録係', lines:[
      {when:{flag:'ch2_towerPaid'}, text:[
        ...
        '「……『前より 薄し』とも」']},
      {when:{flag:'ch2_skyeaterDown'}, text:[
        '「雲が 戻って います。上で 何か あったのですね」',
        '「見張りに 報せて ください。この 階の 北がわに います」',
        '「綴りは その あとで お渡しします」']},
  ...
  // ============ 第3章：雲海港 ============
  sky_port: [
    {at:'8,13', spr:'pipeobj', name:'降下用の 舟', lines:[
      {when:{flag:'ch3_landed'}, text:['桟橋の 先に、舟が 繋いで ある。']},
      {text:['桟橋の 先に、小さな 舟が 繋いで ある。',
             '木でも 鉄でも ない。白い、軽い 何か で できている。']}]},
    {at:'10,13', spr:'pipeobj', name:'降下用の 舟', lines:[
      {when:{flag:'ch3_landed'}, text:['桟橋の 先に、舟が 繋いで ある。']},
      {text:['桟橋の 先に、小さな 舟が 繋いで ある。',
             '木でも 鉄でも ない。白い、軽い 何か で できている。']}]},
    {at:'9,13', spr:'pipeobj', name:'降下用の 舟', lines:[
      {when:{flag:'ch3_landed'}, text:['桟橋の 先に、舟が 繋いで ある。']},
      {text:['桟橋の 先に、小さな 舟が 繋いで ある。',
             '木でも 鉄でも ない。白い、軽い 何か で できている。']}]},
    {at:'5,9', spr:'elder', name:'港の 番人', lines:[
      ...
      {text:['番人「わしは 四十年 この 舟を 磨いてきた」',
             '番人「五地方を 巡る 舟は これ ひとつ。動いた ことは いちども ない」',
             '番人「……乗るなら 南の 桟橋だ。板を たどって 突きあたりまで」']}]},
    {at:'13,9', spr:'guardA', name:'港の 衛兵', lines:[
      {text:['「降下の 許可は 出とる。……珍しい ことだ」',
             '「五地方を 巡る 降下は 四十年 ぶりだと 聞いた」']}]},
    {at:'3,5', spr:'villagerA', name:'荷役', lines:[
      {text:['「荷は 積んだ。灯りと 薬と 布だ」',
             '「毎年 同じ もの。……向こうが 何を 欲しがるかは 知らん」',
             '「毎年の 便は 受け渡し場までだ。今年は 谷の 奥まで 行くと 聞いた」']}]},
    {at:'17,5', spr:'villagerB', name:'雲見の 老人', lines:[
      {text:['「舟が 動くのを 見たのは 初めてだ」',
             '「あれは 飾りだと 思うとった」']}]},
  ],
  ...
  peak_village: [
    ...
    {at:'13,12', spr:'villagerB', name:'山の 鍛冶', lines:[
      {text:['鍛冶「空の 鋼を 見た ことが ある」',
             '鍛冶「十一年 前、荷の ひとつが 谷に 落ちた」',
             '鍛冶「洞の 学者どのが 預かって おると 聞いた」']}]},
  ],
  ...
  zenos_cave: [
    ...
    {at:'9,6', spr:'amane', name:'白い 装束の 娘', lines:[
      {when:{flag:'ch3_mountEar'}, text:[
  ...
  // ============ 第3章：湧き水の町 ============
  well_town: [
    {at:'21,5', spr:'captain', name:'東の 集落長', lines:[
      ...
      {when:{flag:'ch3_wellTold'}, text:[
        '東の 長「水路図だ。オアシスの ほとりの 井戸から 下りられる」']},
  ...
  ice_camp: [
    ...
    {at:'13,5', spr:'villagerB', name:'谷の 鍛冶', lines:[
      {text:['鍛冶「空の 鋼か。……見たことは ある」',
             '鍛冶「十五年ほど 前、若い 男が この 谷を 通った」',
             '鍛冶「赤子を 抱いとった。空から 迎えが 来るのを 待つと」',
...
const QUESTS = {
  ...
  ch3_q2_well: {
    ...
    giver:'東の 集落長',
    desc:'オアシスの 井戸から 地下水路へ。三の 水脈を さがす。',
    steps:[
