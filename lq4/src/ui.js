...
//   mix＝ほかの アプリの 音楽を 止めずに 鳴らす（音楽アプリを 聞きながら あそぶ）
const snd = {bgm:0.8, se:0.8, mix:false};
try{
  ...
  if(raw){ const o=JSON.parse(raw);
    ...
    if(typeof o.mix==='boolean') snd.mix=o.mix; }
}catch(e){}

// ★ほかの アプリの 音楽を 止めるか どうかは、ブラウザの「音の 使いかた」で きまる。
//   'ambient'  … ほかの 音と まざる。音楽アプリは 止まらない
//   'playback' … この ページの 音だけに する（ほかは 止まる。ふつうの ゲーム）
//   iOS16.4 より 古い ブラウザには この しくみが ない。
function audioMixSupported(){
...
function openSound(){
  const mixLabel = !audioMixSupported() ? 'ほかの 音楽と 混ぜる　（使えません）'
                 : (snd.mix ? 'ほかの 音楽と 混ぜる　オン' : 'ほかの 音楽と 混ぜる　オフ');
  const items = ['音楽　'+bar(snd.bgm), '効果音　'+bar(snd.se), mixLabel, '戻る'];
  menu(items, '音の 設定', (sel)=>{
    if(sel===0) tuneSound('bgm', '音楽');
    else if(sel===1) tuneSound('se', '効果音');
    else if(sel===2) toggleMix();
    else C.G.mode='field';
  });
}
// ★音楽アプリを 聞きながら あそぶ ための きりかえ
function toggleMix(){
  if(!audioMixSupported()){
    msg2(['この ブラウザでは まだ 使えません。',
          'ほかの アプリの 音楽は 止まって しまいます。',
          '（iOS 16.4 より あとの Safari で 使えます）'], ()=>{ openSound(); });
    return;
  }
  snd.mix = !snd.mix;
  const lines = [];
  if(snd.mix){
    // ★混ぜる ときに ゲームの 音楽が そのままだと 二重に なって うるさい。
    //   いちど 消して、ほしい 人だけ 上げ直せる ように する。
    if(snd.bgm > 0){ snd.bgm = 0; lines.push('ゲームの 音楽を 消しました。'); }
    lines.unshift('ほかの アプリの 音楽を 止めなく なりました。');
    lines.push('効果音は そのまま 鳴ります。',
               '両方 聞きたい ときは 「音楽」を 上げて ください。');
  }else{
    lines.push('ふつうの ゲームの 音に 戻しました。',
               'ほかの アプリの 音楽は 止まります。');
