...
function buildAtlas(){
  ...
  // ★上の 建物：みがいた 白石に 金の 帯、青い ガラス窓。

  //   下層の 割れた 石、中層の 磨き石、上層の 白亜、と 三段で 差が つく。
  atlas.richwall = tileSet((g,s,v)=>{
    // ★白亜の 石づみ。上下に 太い 蛇腹（じゃばら）、まん中に 大きな 窓。
    //   まえは 細い 金の 線と 小さな 窓で、しま模様に 見えて いた。
    R(g,0,0,s,s,P16.up1);
    // 石づみ（大きめ・ずらしつみ）
    const brick=(bx,by,bw)=>{
      R(g,bx,by,bw,7,P16.up2);
      R(g,bx,by,bw,1,P16.up3);          // うえの かど：光
      R(g,bx,by+6,bw,1,P16.up0);        // したの かど：影
      R(g,bx+bw-1,by,1,7,P16.up0);      // つぎめ
    };
    const off = (v%2)*4;
    for(let by=0; by<16; by+=8){
      for(let bx=-8+off; bx<16; bx+=9) brick(bx,by,9);
    }
    // ★上の 蛇腹（じゃばら）：白石の 出っぱりに、細い 金の 線を 一本。
    //   金を 太く すると 金ばかりが 目に つき、石が 見えなく なる。
    R(g,0,0,s,3,P16.up3);
    R(g,0,0,s,1,'#ffffff');
    R(g,0,2,s,1,P16.gold1);
    R(g,0,3,s,1,P16.up0);
    // ★下の 蛇腹
    R(g,0,13,s,1,P16.gold1);
    R(g,0,14,s,2,P16.up3);
    R(g,0,15,s,1,P16.up0);
    // ★大きな アーチ窓（1ますに ひとつ、たてに 長い）
    if(v%2===0){
      const wx=5, wy=4, ww=6, wh=8;
      // 窓わく
      R(g,wx-1,wy-1,ww+2,wh+2,P16.up0);
      R(g,wx-1,wy-1,ww+2,1,P16.gold2);
      // ガラス
      R(g,wx,wy,ww,wh,P16.win0);
      R(g,wx,wy,ww,1,'#1b3f6b');
      // アーチの まる み
      P(g,wx,wy,P16.up0); P(g,wx+ww-1,wy,P16.up0);
      // 光の さしこみ
      R(g,wx+1,wy+2,2,4,P16.win1);
      P(g,wx+1,wy+2,P16.win2);
      // 桟（さん）
      R(g,wx+ww/2,wy,1,wh,P16.up0);
      R(g,wx,wy+4,ww,1,P16.up0);
      // 窓下の 出っぱり
      R(g,wx-2,wy+wh+1,ww+4,1,P16.up3);
      R(g,wx-2,wy+wh+2,ww+4,1,P16.up0);
    }else{
      // 窓の ない ますは 柱で 引きしめる
      R(g,2,3,2,10,P16.up3);
      R(g,4,3,1,10,P16.up0);
      R(g,12,3,2,10,P16.up3);
      R(g,14,3,1,10,P16.up0);
      R(g,2,4,2,1,P16.gold1);
      R(g,12,4,2,1,P16.gold1);
    }
  });
...
// ---------------- タイル → え の わりあて ----------------
// ★大きな 噴水。
//   ますごとに 文字を 増やすと、大きさを 変える たびに 増えて いく。
//   かわりに 「F を ならべた 四角の 大きさ」で 形が きまる ように する。
//   2×2 なら F を 4つ、3×3 なら F を 9つ ならべる だけ。
const fountCache = {};
function fountPart(tw, th, cx, cy){
  const key = tw+'x'+th+':'+cx+','+cy;
  if(fountCache[key]) return fountCache[key];
  ...
  const t = tile((g,s)=>{
    ...
    // 楕円を よこ線の つみかさねで かく（角が 丸く なる）
    const ellip = (cx0, cy0, rx, ry, col)=>{
      for(let dy=-ry; dy<=ry; dy++){
        const k = 1 - (dy*dy)/(ry*ry);
        if(k<=0) continue;
        const w = Math.round(rx*Math.sqrt(k));
        B(cx0-w, cy0+dy, w*2, 1, col);
      }
    };
    const ST0='#7f95c0', ST1='#a9bcd8', ST2='#d9e3f0', ST3='#f2f6fc';
    R(g,0,0,s,s,'#c9d8ee');
    const CX = PW/2;
    const bowlY = PH*0.56;                 // 水盤の 中心（たて）
    const rx    = PW*0.42, ry = PH*0.24;   // 水盤の 大きさ
    // 台座（下ほど 細い 三だん）
    ellip(CX, bowlY+ry*1.15, rx*0.52, ry*0.22, ST0);
    ellip(CX, bowlY+ry*1.55, rx*0.34, ry*0.20, '#6f86ab');
    ellip(CX, bowlY+ry*1.95, rx*0.70, ry*0.22, ST0);
    // 水盤の 外（下の ふくらみ → うえの ふち の 順に かさねる）
    ellip(CX, bowlY+ry*0.30, rx, ry*0.86, ST0);
    ellip(CX, bowlY,          rx, ry,      ST1);
    ellip(CX, bowlY-ry*0.10,  rx, ry*0.94, ST2);
    ellip(CX, bowlY-ry*0.16,  rx*0.97, ry*0.88, ST1);
    // 水
    ellip(CX, bowlY-ry*0.18, rx*0.80, ry*0.70, ST0);
    ellip(CX, bowlY-ry*0.22, rx*0.76, ry*0.64, '#2f6ea8');
    ellip(CX, bowlY-ry*0.30, rx*0.66, ry*0.50, '#5fb4ee');
    ellip(CX, bowlY-ry*0.38, rx*0.40, ry*0.30, '#a8e2ff');
    // 中央の 柱
    const pw = Math.max(3, PW*0.10);
    B(CX-pw/2, bowlY-ry*1.05, pw, ry*1.0, ST1);
    B(CX-pw/2, bowlY-ry*1.05, 1, ry*1.0, ST2);
    B(CX+pw/2-1, bowlY-ry*1.05, 1, ry*1.0, ST0);
    ellip(CX, bowlY-ry*1.05, pw*0.9, Math.max(2,ry*0.16), ST2);
    // 噴きあがる 水
    B(CX-pw*0.34, 0, Math.max(2,pw*0.68), bowlY-ry*0.95, '#eafaff');
    B(CX-pw*0.60, PH*0.04, Math.max(1,pw*0.26), bowlY-ry*1.1, '#d9e3f0');
    B(CX+pw*0.34, PH*0.04, Math.max(1,pw*0.26), bowlY-ry*1.1, '#d9e3f0');
    // 左右へ こぼれる すじ
    B(CX-pw*1.5, PH*0.10, 1, PH*0.16, '#a8e2ff');
    B(CX+pw*1.5, PH*0.10, 1, PH*0.16, '#a8e2ff');
    B(CX-pw*2.2, PH*0.16, 1, PH*0.10, '#cfe2ff');
    B(CX+pw*2.2, PH*0.16, 1, PH*0.10, '#cfe2ff');
    // しぶき
    [[0.34,0.52],[0.68,0.55],[0.46,0.64],[0.58,0.60],[0.40,0.58]]
      .forEach(([fx,fy])=>B(PW*fx, PH*fy, 1, 1, '#ffffff'));
  });
  fountCache[key] = t;
  return t;
}
...
function gatePart(tw, th, cx, cy){
  ...
  const t = tile((g,s)=>{
    ...
    const tw2 = Math.round(PW*0.26);        // 塔の はば
    const roofH = Math.round(PH*0.13);      // 屋根の たかさ
    const top   = roofH;                    // 石づみの はじまり

    // ---- 左右の 塔 ----
    [0, PW-tw2].forEach((tx)=>{
      B(tx, top, tw2, PH-top, U1);
      // 石づみ（ずらして つむ。よこ縞に 見えない ように）
      let row=0;
      for(let by=top+1; by<PH-1; by+=6){
        const off = (row%2)*4;
        for(let bx=tx-4+off; bx<tx+tw2; bx+=8){
          const x1=Math.max(tx, bx), x2=Math.min(tx+tw2, bx+8);
          if(x2-x1<=0) continue;
          B(x1, by, x2-x1, 5, U2);
          B(x1, by, x2-x1, 1, U3);
          B(x1, by+4, x2-x1, 1, U0);
          B(x2-1, by, 1, 5, U0);
        }
        row++;
      }
      B(tx, top, 1, PH-top, U3);
      B(tx+tw2-1, top, 1, PH-top, U0);
      // 青い 窓（たてに 2つ）
      [0.30, 0.58].forEach(f=>{
        const wx=tx+Math.round(tw2*0.28), wy=Math.round(top+(PH-top)*f);
        const ww=Math.round(tw2*0.44), wh=Math.round(PH*0.11);
        B(wx-1, wy-1, ww+2, wh+2, U0);
        B(wx, wy, ww, wh, '#2a5c96');
        B(wx, wy, ww, 1, '#1b3f6b');
        B(wx+1, wy+1, 2, Math.max(2,wh-3), '#5fb4ee');
        B(wx-1, wy-2, ww+2, 1, G1);
      });
    });

    // ---- 屋根（左右の 塔の 上）----
    //   ★まえは 端から はみ出して 浮いて 見えた。
    //     塔の はばの 中に おさめ、下ほど 広い 台形に する。
    [0, PW-tw2].forEach((tx)=>{
      const rows = Math.max(3, roofH);
      for(let k=0;k<rows;k++){
        // k=0 が てっぺん（せまい）、k=rows-1 が いちばん 下（塔の はば）
        const f = k/(rows-1);
        const w = Math.round(tw2*(0.34 + 0.66*f));
        const x = tx + Math.round((tw2-w)/2);
        B(x, roofH-rows+k, w, 1, k<rows*0.45 ? G2 : G1);
      }
      // 軒（のき）：屋根の 下だけ。塔の はば ぜんぶに 引くと 平らに 見える。
      B(tx+Math.round(tw2*0.10), roofH-1, Math.round(tw2*0.80), 1, G0);
    });

    // ---- まん中の はり（まぐさ）----
    const lx = tw2-1, lw = PW-tw2*2+2;
    const ly = Math.round(PH*0.16), lh = Math.round(PH*0.10);
    B(lx, ly, lw, lh, U1);
    B(lx, ly, lw, 1, U3);
    B(lx, ly+lh-2, lw, 1, G1);
    B(lx, ly+lh-1, lw, 1, G0);
    // 紋章（光珠）
    const mx = Math.round(PW/2), my = ly+Math.round(lh/2);
    B(mx-4, my-4, 8, 8, G1);
    B(mx-3, my-3, 6, 6, G2);
    B(mx-2, my-2, 4, 4, '#2a5c96');
    B(mx-2, my-2, 4, 2, '#5fb4ee');
    B(mx-1, my-2, 2, 1, '#eafaff');

    // ---- 門の 扉（二枚）----
    const dx0 = tw2, dw = PW-tw2*2, dy0 = ly+lh, dh = PH-dy0-Math.round(PH*0.05);
    B(dx0, dy0, dw, dh, '#4a3b22');
    B(dx0, dy0, dw, 1, '#2b2213');
    // たての 板目（扉の 中だけに おさめる）
    for(let vx=dx0+2; vx+4<=dx0+dw-2; vx+=5){
      B(vx, dy0+1, 3, dh-2, '#5d4a2b');
      B(vx+3, dy0+1, 1, dh-2, '#33291a');
    }
    // 金の 帯（うすく 3本）
    [0.16, 0.46, 0.76].forEach(f=>{
      const by = dy0+Math.round(dh*f);
      B(dx0+1, by, dw-2, 2, G1);
      B(dx0+1, by, dw-2, 1, G2);
    });
    // まん中の 合わせ目と 取っ手
    B(mx-1, dy0, 2, dh, '#2b2213');
    B(mx-5, dy0+Math.round(dh*0.46), 3, 3, G2);
    B(mx+2, dy0+Math.round(dh*0.46), 3, 3, G2);
    // 扉の わく（石の 柱。壁と つながって 見える ように）
    B(dx0-1, dy0-1, 2, dh+1, U1);
    B(dx0-1, dy0-1, 1, dh+1, U2);
    B(dx0+dw-1, dy0-1, 2, dh+1, U1);
    B(dx0+dw, dy0-1, 1, dh+1, U0);

    // ---- 石段 ----
    B(dx0-2, PH-Math.round(PH*0.05), dw+4, Math.round(PH*0.03), U2);
    B(dx0-2, PH-Math.round(PH*0.05), dw+4, 1, U3);
    B(dx0-4, PH-2, dw+8, 2, U0);
  });
  gateCache[key] = t;
  return t;
}
...
function fountAt(x, y){
  const m = C.MAPS && C.MAPS[curMap];
  if(!m) return fountPart(1,1,0,0);
  const at = (xx,yy)=>((m.tiles[yy]||'')[xx]) || '#';
  let x0=x; while(at(x0-1,y)==='F') x0--;
  let y0=y; while(at(x,y0-1)==='F') y0--;
  let tw=0; while(at(x0+tw,y0)==='F') tw++;
  let th=0; while(at(x0,y0+th)==='F') th++;
  return fountPart(tw, th, x-x0, y-y0);
}


function tileArt(ch, theme, tx, ty){
...
// ★画面の 上の 隠れる 帯（iPhone の 時刻・ダイナミックアイランド）。キャンバスの 画素で。
//   ★地図の 端で カメラを 止めると、いちばん 上の 行が この 帯の 裏に 入り、
//     地上の 舟（2行め）が ちょうど アイランドの 裏に 隠れて いた。
let safeTopPx = 0;
function measureSafeTop(r, dpr){
  try{
    ...
    const inset = d.getBoundingClientRect().height || 0;
    d.remove();
    return Math.max(0, Math.round((inset - r.top) * dpr));   // キャンバスに かかる ぶんだけ
  }catch(e){ return 0; }
}
function resize(){
  if(!cv) return;
  ...
  safeTopPx = measureSafeTop(r, dpr);
}
...
// ★技ごとの 見た目。
//   まえは 技も 通常こうげきも おなじ ひとつの えんしゅつ だった ため、
//   何を したのか 画面から わからなかった。
//   ここでは 「粒の 色・形・とび方」と「切りきずの 線」を 技ごとに かえる。
const FX_STYLE = {
  // 剣（イオ）：金と 白。たてに 走る 光
  ikkiuchi:   {col:['#ffe6a8','#f0c058'], n:10, spread:200, up:200, slash:'v',  flash:0.45, shake:0.24},
  kabutowari: {col:['#cfe0ff','#8fa6c8'], n:14, spread:120, up:70,  slash:'down',flash:0.5,  shake:0.34, ring:1},
  renzan:     {col:['#ffffff','#ffe6a8'], n:16, spread:300, up:180, slash:'x',  flash:0.55, shake:0.30, hits:2},
  yoroikudaki:{col:['#cfe0ff','#7f95c0'], n:18, spread:140, up:60,  slash:'down',flash:0.6,  shake:0.42, ring:1},
  tenkuuken:  {col:['#eafaff','#a8e2ff'], n:20, spread:260, up:300, slash:'v',  flash:0.7,  shake:0.36, beam:1},
  kenpunagi:  {col:['#ffffff','#cfe0ff'], n:22, spread:420, up:120, slash:'h',  flash:0.6,  shake:0.34, wide:1},
  seikousen:  {col:['#ffffff','#ffd257'], n:26, spread:340, up:360, slash:'v',  flash:0.85, shake:0.5,  beam:1},
  amagakeken: {col:['#eafaff','#ffffff'], n:30, spread:380, up:420, slash:'x',  flash:0.9,  shake:0.55, beam:1},
  seikou_kiwa:{col:['#ffffff','#ffd257'], n:34, spread:520, up:300, slash:'x',  flash:1.0,  shake:0.6,  wide:1, beam:1},
  // 槍（セレン）：銀と 水色。するどい 点
  shippuutsuki:{col:['#dff0ff','#9ad0ff'], n:8,  spread:120, up:60,  slash:'stab', flash:0.4, shake:0.20},
  yoroidooshi:{col:['#cfe0ff','#7f95c0'], n:12, spread:100, up:50,  slash:'stab', flash:0.5, shake:0.34, ring:1},
  kagerou:    {col:['#c8a8ff','#9ad0ff'], n:14, spread:180, up:90,  slash:'stab', flash:0.45,shake:0.24, haze:1},
  nidantsuki: {col:['#ffffff','#9ad0ff'], n:14, spread:160, up:80,  slash:'stab', flash:0.55,shake:0.28, hits:2},
  nagiharai:  {col:['#dff0ff','#a8c8e8'], n:20, spread:420, up:90,  slash:'h',    flash:0.55,shake:0.32, wide:1},
  ryuuseitsuki:{col:['#ffffff','#a8e2ff'],n:22, spread:200, up:260, slash:'stab', flash:0.7, shake:0.36, beam:1},
  kansen:     {col:['#eafaff','#7fd4ff'], n:24, spread:180, up:300, slash:'stab', flash:0.8, shake:0.44, beam:1},
  amatsuranuki:{col:['#ffffff','#7fd4ff'],n:28, spread:220, up:360, slash:'stab', flash:0.85,shake:0.5,  beam:1},
  ginu:       {col:['#dff0ff','#ffffff'], n:26, spread:460, up:200, slash:'rain', flash:0.6, shake:0.34, wide:1},
  ryuuseigun: {col:['#ffffff','#a8e2ff'], n:32, spread:520, up:320, slash:'rain', flash:0.8, shake:0.46, wide:1},
  kansen_kiwa:{col:['#ffffff','#eafaff'], n:36, spread:260, up:440, slash:'stab', flash:1.0, shake:0.6,  beam:1},
  // 術（ノエ・アマネ）
  spark:      {col:['#fff2b0','#ffd257'], n:14, spread:220, up:200, slash:'none', flash:0.5, shake:0.22, ring:1},
  spara:      {col:['#fff2b0','#ffd257'], n:20, spread:300, up:260, slash:'none', flash:0.65,shake:0.3,  ring:1},
  sparga:     {col:['#ffffff','#ffd257'], n:26, spread:420, up:320, slash:'none', flash:0.8, shake:0.4,  ring:1, wide:1},
  hiira:      {col:['#c8a8ff','#8f7fd0'], n:14, spread:200, up:160, slash:'none', flash:0.45,shake:0.2,  haze:1},
  // 光の 大技（イオ・アマネで 共通）
  holyray:    {col:['#ffffff','#ffe6a8'], n:30, spread:360, up:400, slash:'v',    flash:0.9, shake:0.5,  beam:1, ring:1},
  judgment:   {col:['#ffffff','#a8e2ff'], n:36, spread:520, up:420, slash:'x',    flash:1.0, shake:0.62, beam:1, wide:1, ring:1},
};
const FX_DEFAULT = {col:['#f0c058','#e06038'], n:12, spread:260, up:220,
                    slash:'none', flash:0.5, shake:0.26};
function fxStyle(key){ return (key && FX_STYLE[key]) || FX_DEFAULT; }

// 切りきず・突きの 線（技の 形を 見せる）
let bSlash = null;
function burst2D(target, key){
  const st = fxStyle(key);
  const f = target ? bFoes.find(x=>x.enemy===target) : bFoes[0];
  const cx0 = f ? f._cx : W/2, cy0 = f ? f._cy : H/2;
  const wide = st.wide ? 1.6 : 1;
  for(let i=0;i<st.n;i++){
    const a = (nz(i,1,11)-0.5);
    bParts.push({
      x: cx0 + (st.wide ? a*W*0.35 : 0),
      y: cy0 + (st.slash==='rain' ? -H*0.2*nz(i,4,7) : 0),
      vx: a*st.spread*wide,
      vy: -(40 + nz(i,2,12)*st.up),
      life: st.beam ? 0.7 : 0.5,
      col: i%2 ? st.col[0] : st.col[1],
      sz: st.beam ? 3 : 2,
    });
  }
  if(st.slash!=='none') bSlash = {t:0, kind:st.slash, x:cx0, y:cy0, col:st.col[0], wide:!!st.wide};
  if(st.haze){
    for(let i=0;i<10;i++)
      bParts.push({x:cx0+(nz(i,5,3)-0.5)*90, y:cy0+(nz(i,6,4)-0.5)*70,
                   vx:(nz(i,7,5)-0.5)*40, vy:-20, life:0.9, col:'#c8a8ff', sz:4});
  }
  if(st.ring){
    for(let i=0;i<14;i++){
      const th = i/14*6.283;
      bParts.push({x:cx0, y:cy0, vx:Math.cos(th)*180, vy:Math.sin(th)*120,
                   life:0.45, col:st.col[1], sz:2});
    }
  }
}
...
function drawBattle(dt, time){
  ...
  if(bFx){
    ...
    if(s.kind==='enter'){
      ...
      const st = fxStyle(s.key);
      const hits = st.hits || 1;
      if(s.t<0.16) zoom = 1 + s.t*0.5;
      else if(s.t<0.24){
        if(!s.hit){ s.hit=1; bShake=st.shake; bFlash=st.flash; burst2D(s.target, s.key); }
        zoom=1.08;
      }
      else if(s.t<0.5){
        // ★2だん技は もう一度 きらめく
        if(hits>1 && s.hit<2 && s.t>0.30){
          s.hit=2; bShake=st.shake*0.8; bFlash=st.flash*0.7; burst2D(s.target, s.key);
        }
        zoom = 1.08 - (s.t-0.24)*0.32;
      }
  ...
  if(bSlash){
    bSlash.t += dt;
    const a = Math.max(0, 1 - bSlash.t/0.26);
    if(a<=0) bSlash = null;
    else{
      const L = (bSlash.wide ? W*0.46 : W*0.20), th = Math.max(2, sc*3);
      cx.save();
      cx.globalAlpha = a;
      cx.fillStyle = bSlash.col;
      const X = bSlash.x, Y = bSlash.y - sc*14;
      const bar = (dx,dy,len,w)=>{
        cx.save(); cx.translate(X,Y); cx.rotate(Math.atan2(dy,dx));
        cx.fillRect(-len/2, -w/2, len, w); cx.restore();
      };
      if(bSlash.kind==='v')      bar(0.3,1, L, th);              // たてに 振りおろす
      else if(bSlash.kind==='h') bar(1,0, L*1.4, th);            // よこに 薙ぐ
      else if(bSlash.kind==='down'){ bar(0.6,1, L, th*1.6); }    // 打ちおろし（太い）
      else if(bSlash.kind==='x'){ bar(1,0.9, L, th); bar(1,-0.9, L, th); }
      else if(bSlash.kind==='stab'){                             // 突き（点から 線）
        cx.save(); cx.translate(X,Y);
        cx.fillRect(-L*0.5, -th*0.5, L*(0.4+0.6*(1-a)), th);
        cx.restore();
      }
      else if(bSlash.kind==='rain'){                             // ふりそそぐ
        for(let i=0;i<7;i++){
          const px = X + (nz(i,8,2)-0.5)*W*0.6;
          cx.fillRect(Math.round(px), Math.round(Y-H*0.3+H*0.5*(1-a)), th, sc*10);
        }
      }
      cx.restore();
      cx.globalAlpha = 1;
  ...
  bParts = bParts.filter(p=>{
    ...
    cx.fillStyle = p.col;
    const z = sc*(p.sz||2);
    cx.fillRect(Math.round(p.x), Math.round(p.y), z, z);
    return true;
  });
...
function draw(dt, time, actors){
  ...
  // ★たては 上の 隠れる 帯を のぞいた 高さ（availH）で 考える
  const top0 = Math.min(safeTopPx, Math.floor(H*0.2));
  const availH = H - top0;
  const viewW = W/ts, viewH = availH/ts;
  ...
  const oy = (mapH <= availH) ? top0 + Math.round((availH-mapH)/2)
                              : Math.round(top0 + availH/2 - camY*ts);
